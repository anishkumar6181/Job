...
export const JobsContextProvider = ({ children }) => {
  ...
  const updateJob = async (jobId, updatedData) => {
    try {
      // Add proper error handling for missing jobId
      if (!jobId) {
      ...
      // Update both jobs and userJobs states
      setJobs(prevJobs =>
        prevJobs.map(job => 
  ...
  const createAdminJob = async (jobData) => {
    try {
      const { data } = await axios.post("/admin/jobs/create", jobData, {
        headers: {
          'Content-Type': 'application/json'
        },
        withCredentials: true
      ...
      setAdminJobs(prev => [data.job, ...prev]);
      toast.success('Job created successfully');
      ...
      console.error('Admin job creation error:', {
        message: errorMessage,
        status: error.response?.status,
        data: error.response?.data
      });
